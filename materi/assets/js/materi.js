...
let IMG_BASE = "";
if (_initialSrc) {
  IMG_BASE = _initialSrc.replace(/\d+\.jpg$/, "");
}
if (!IMG_BASE) IMG_BASE = "../../assets_materi/emisi";

function setSlide(n) {
  const target = Math.min(total, Math.max(1, n));

  // Always keep UI in sync (even if same slide)
  ...
  // If no change in slide index, nothing more to do
  ...
  setTimeout(() => {
    if (slideImg) slideImg.src = `${IMG_BASE}${current}.jpg`;
  }, 80);
}
...
if (slideImg) {
  ...
  // sync UI state (this will be a no-op for the same slide)
  setSlide(current);
}
